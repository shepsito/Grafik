...
class NotificationApp(MDApp):
    def build(self):
        # Модерна Тъмна тема с приятен Teal (синьо-зелен) цвят
        ...
        # Контейнер за съдържанието под лентата
        content_layout = MDBoxLayout(orientation="vertical", padding=20, spacing=20)
    ...
    def check_schedule(self, dt):
        ...
        if is_last_tuesday_of_quarter(now):
            msg = f"Съоръжение: МЗ\nПроверка: Проверка АВР сборки на 0,4кВ захранвани от 23 and 24 БН\nСмяна: {shift}"
